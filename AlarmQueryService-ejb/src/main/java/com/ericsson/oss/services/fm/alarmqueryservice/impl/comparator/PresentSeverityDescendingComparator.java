 ...
 * PseudoSeverity(PreviousSeverity_PresentSeverity) attribute.
 *
 *
 */
public class PresentSeverityDescendingComparator implements Comparator<AlarmRecord> {

    String attribute;

    public PresentSeverityDescendingComparator(final String attribute) {
    ...
    public int compare(final AlarmRecord firstAlarmRecord, final AlarmRecord secondAlarmRecord) {
        ...
        return PseudoSeverities.PSEUDO_SEVERITIES_MAP.get(secondPseudoSeverity)
                .compareTo(PseudoSeverities.PSEUDO_SEVERITIES_MAP.get(firstPseudoSeverity));
    }
}