 ...
 *
 */
public class PreviousSeverityAscendingComparator implements Comparator<AlarmRecord> {

    String attribute;

    public PreviousSeverityAscendingComparator(final String attribute) {
        this.attribute = attribute;
    }

    @Override
    public int compare(final AlarmRecord firstAlarmRecord, final AlarmRecord secondAlarmRecord) {
        return PseudoSeverities.PSEUDO_SEVERITIES_MAP.get(firstAlarmRecord.getPreviousSeverity().toString()).compareTo(
                PseudoSeverities.PSEUDO_SEVERITIES_MAP.get(secondAlarmRecord.getPreviousSeverity().toString()));
    }
}